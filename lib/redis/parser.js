...
exports.returnReply = function (reply) {
  ...
  if (this.condition.subscriber) {
    ...
    // logData('reply from redis', {command: item.command.name, slot: item.command.slot, args: item.command.args, reply: reply && reply.toString(), now: Date.now(), id: item.command.id || 'missing' })
